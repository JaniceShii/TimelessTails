...
import javax.swing.*;          // actionEvent and ActionListener used to handle button clicks or user input
...
public class InventoryUI extends AbstractScreen {
    ...
    // createBackground method creates the background of the window
    ...
    // createUseItemButton creates the button to use the item
    ...
    // createExitIventoryButton creates the button to exit the inventory
    ...
    // createTitleText draws the inventory title
    ...
    // updateInventoryDisplay method updates buttons with item data from the item array
    public void updateInventoryDisplay() {
        ...
        // create the buttons for the items slots
        ...
        // clear all buttons icons and text
        ...
        for (Map.Entry<String, Item> entry : itemInventory.getItemMap().entrySet()){
            ...
            if (item.getNumItems() > 0){
                ...
                if (imageName != null){
                    // when there is an item then it will load the image into the slot
                    ImageIcon unscaledImg = new ImageIcon(getClass().getClassLoader().getResource("itemImagesAssets/" + imageName));
                ...
                // when there is no item then create an empty cell
                itemSlots[i].setIcon(null);
                itemSlots[i].setText("");
            }
            i++;
    ...
    // itemDisplayPanel that daws all item count rectangles and item names in a loop
    ...
    // selectItem method handles item selection
    // when user selects an item, it will be highlighted and a popup message appears describing the item
    ...
    // useItem method handles item usage
    // once player selects an item and clicks the use item button
    // then it will update stats and close the inventory
